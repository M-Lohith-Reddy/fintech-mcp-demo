"""
MCP Client Wrapper - Multi-Server Support
Supports: GST Server + Onboarding Info Server
"""
...
class MCPClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.error(f"✗ {self.server_name} failed: {e}")
            raise

    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Call MCP tool using fresh connection"""
...
# ═══════════════════════════════════════════════════════════════════════
# TWO MANAGERS: GST + ONBOARDING INFO
# ═══════════════════════════════════════════════════════════════════════
...
redbus_client_manager = MCPClientManager(
    server_module = "mcp_server.redbus_server",
    server_name   = "RedBus Redirect"
)

# Backward compatibility
mcp_client_manager = gst_client_manager